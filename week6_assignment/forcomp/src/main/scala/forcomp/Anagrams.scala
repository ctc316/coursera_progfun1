...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    (w.toLowerCase groupBy identity).toList.sorted map {case (ch, str) => (ch, str.length)}
  }
...
//    (dictionaryByOccurrences withDefaultValue List()) (wordOccurrences(word))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences.foldRight( List[Occurrences](List()) ) {
      case ((ch, n), acc) => acc ++ ( for {comb <- acc; i <- 1 to n} yield (ch, i) :: comb)
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.foldLeft(x.toMap) {
      case (acc_map, (ch, n)) => {
        val remain = acc_map(ch) - n
        if (remain > 0) acc_map.updated(ch, remain)
        else acc_map - ch
      }
    }.toList.sorted
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def iter(occrs: Occurrences): List[Sentence] = {
      if (occrs.isEmpty) List(Nil)
      else for {
        comb <- combinations(occrs)
        word <- dictionaryByOccurrences getOrElse (comb, Nil)
        sentence <- iter(subtract(occrs,wordOccurrences(word)))
      } yield { word :: sentence }
    }
