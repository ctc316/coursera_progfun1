...
//    (Map[Char,Int]() /: chars)(incr).iterator.toList
    ...
    chars.foldLeft(Map[Char,Int]())((map, char)=>{
      val count = (map.get(char)).getOrElse(0) + 1
      map + ((char, count))
    }).iterator.toList
  }
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      freqs.sortWith((tuple1, tuple2) => tuple1._2 < tuple2._2).map((tuple) => Leaf(tuple._1, tuple._2))
    }
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      case leftTree :: rightTree :: remain => (makeCodeTree(leftTree, rightTree) :: remain)
   ...
   */
    def until(condition: List[CodeTree]=>Boolean, doFunc: List[CodeTree]=>List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
      if (condition(trees)) trees
      else until(condition, doFunc)(doFunc(trees))
    }
  /**
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def traverse(remainTree: CodeTree, bits: List[Bit]): List[Char] = remainTree match {
